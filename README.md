...
The ESP32 then communicates these commands to the STM32 microcontroller, enabling control over both motor and sound functions. <br>
...
the control unit, interfacing with DC motors through a motor driver and the speaker and amplifier directly. All commands
are sent through the Python client to the ESP32 to be sent to the STM32. This separation enables the STM32 to achieve low-latency operation by offloading network responsibilities to the ESP32.
...
|W|Drive Forward|
|A|Left Turn|
|S|Deccelerate (can be until stop)|
|D|Right Turn|
...
## Software
- C for STM32/ESP32
- STM32's PWM for Motor Control
- STM32's DMA + DAC for Sound Control
